...
public class TicketSpecificationBuilder {
    ...
    private <V> Optional<Specification<Ticket>> createSpecification(V value, Function<V, Specification<Ticket>> function) {
    ...
    private Specification<Ticket> titleContains(String searchTerm) {
        return (root, query, cb) -> cb.like(cb.lower(root.get("title")), "%" + searchTerm.toLowerCase() + "%");
    }
